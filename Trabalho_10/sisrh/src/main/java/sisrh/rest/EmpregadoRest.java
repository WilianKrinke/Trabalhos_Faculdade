...
public class EmpregadoRest {
	...
	public Response listarEmpregadosInativos() throws Exception {
		List<Empregado> lista = Banco.listarEmpregados();
		List<Empregado> listaFiltrada = new ArrayList<>();
		
		for(Empregado empregado : lista) {
			if(empregado.getDesligamento() != null) {listaFiltrada.add(empregado);}
		}
		
		GenericEntity<List<Empregado>> entity = new GenericEntity<List<Empregado>>(listaFiltrada) {
		};
		return Response.ok().entity(entity).build();
